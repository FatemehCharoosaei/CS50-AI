...
def load_data(filename: str) -> tuple[list[EvidenceRow], list[Label]]:
    ...
    with open(filename) as file:
        for row_ in csv.DictReader(file):
            row: RawDataRow = row_  # type: ignore
            Evidence.append(
                (
                    ...
                    ParseBoolToInt(row["Weekend"]),
                )
            )

            Labels.append(ParseBoolToInt(row["Revenue"]))

    return (Evidence, Labels)


def ParseBoolToInt(value: str) -> Literal[0, 1]:
...
def evaluate(Labels: list[Label], predictions: list[Label]) -> tuple[float, float]:
    ...
    correctly_predicted_negative = 0
    ...
    for Label, prediction in zip(Labels, predictions, strict=True):
        if Label == 0:  # expected negative
            expected_negative += 1
            if Label == prediction:
                correctly_predicted_negative += 1
    ...
    return (
        correctly_predicted_positive / expected_positive,
        correctly_predicted_negative / expected_negative,
    )
