...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for prsn in P:
        ...
        num = 1 * (prsn in one_gene) + 2 * (prsn in two_genes)
        have = prsn in have_trait
    ...
    for prsn in Ch:
        ...
        mom = people[prsn]["mother"]
        dad = people[prsn]["father"]

        # as before, evaluate the number of genes for the parents and the child
        # also evaluate whether the child has a trait
        num = 1 * (prsn in one_gene) + 2 * (prsn in two_genes)
        nummom = 1 * (mom in one_gene) + 2 * (mom in two_genes)
        numdad = 1 * (dad in one_gene) + 2 * (dad in two_genes)
        have = prsn in have_trait
        ...
        if num == 0:
            ...
            elif (numdad == 2 and nummom == 0) or (numdad == 0 and nummom == 2):
            ...
            elif (numdad == 0 and nummom == 1) or (numdad == 1 and nummom == 0):
            ...
            elif (numdad == 2 and nummom == 1) or (numdad == 1 and nummom == 2):
                effect = mutation * 0.5
        ...
        # if the child has one gene
            ...
            if (numdad == 0 and nummom == 0) or (numdad == 2 and nummom == 2):
                effect = mutation * (1 - mutation)
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    for prsn in probabilities:
        num = 1 * (prsn in one_gene) + 2 * (prsn in two_genes)
        have = prsn in have_trait
        probabilities[prsn]["gene"][num] += p
        probabilities[prsn]["trait"][have] += p
...
def normalize(probabilities):
    ...
    for prsn in probabilities:
        sum = 0
        for i in range(3):
            sum += probabilities[prsn]["gene"][i]

        for i in range(3):
            probabilities[prsn]["gene"][i] = probabilities[prsn]["gene"][i] / sum

        sum = probabilities[prsn]["trait"][True] + probabilities[prsn]["trait"][False]

        probabilities[prsn]["trait"][True] = probabilities[prsn]["trait"][True] / sum

        probabilities[prsn]["trait"][False] = probabilities[prsn]["trait"][False] / sum
